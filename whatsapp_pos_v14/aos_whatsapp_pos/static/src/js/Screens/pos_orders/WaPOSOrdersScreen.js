odoo.define('aos_whatsapp_pos.WaPOSOrdersScreen', function (require) {
	'use strict';
	...
	class WaPOSOrdersScreen extends PosComponent {
		constructor() {
			super(...arguments);
			this.state = {
				query: null,
		...
		async clickReprint(event){
			...
			await self.rpc({
				...
				//console.log('--clickReprint--',data)
				self.showTempScreen('WaOrderReprintScreen',data);
			});
		}
		...
		get_orders_domain(){
			...
			if (pos_config.pos_session_limit == 'last5o'){
				if (pos_config.show_draft == true){
					if (pos_config.show_posted == true){
						return [['state', 'in', ['draft', 'done']]]; 
					} else {
		...
		get_orders_limit(){
			...
			let current = self.env.pos.pos_session.id;
			let pos_config = self.env.pos.config;
			return pos_config.show_limit;
		}
		async get_pos_orders () {
			//console.log('=get_pos_orders=')
			let self = this;
		...
		showDetails(event){
			...
			console.log('==showDetails==',orderlines)
			let pos_lines = [];
			...
			self.showPopup('WaPosOrdersDetail', {
				'order': event.detail, 
				'orderline':pos_lines,
			});
		}
	...
	WaPOSOrdersScreen.template = 'WaPOSOrdersScreen';
	WaPOSOrdersScreen.hideOrderSelector = true;
	Registries.Component.add(WaPOSOrdersScreen);
	return WaPOSOrdersScreen;
});
